...
export default function BulkAssignModal({ weekDates, onClose, preselectedRecipe = null }) {
        ...
        <div className="fixed top-0 left-0 right-0 z-[60]" style={{ height: '100vh' }}>
            ...
            >
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-slate-700/50 flex-shrink-0">
                    <h2 className="text-lg font-bold text-white">{preselectedRecipe ? 'Sposta Pasto' : 'Aggiungi Pasti'}</h2>
                ...
                >
                    {/* Step 1: Scegli una ricetta (hidden when preselected) */}
                    {preselectedRecipe ? (
                        <div className="flex items-center gap-3 px-3 py-3 rounded-xl bg-green-500/15 border border-green-500/50">
                            <ChefHat size={16} className="text-green-400" />
                            <span className="text-sm font-medium text-green-400 truncate">{preselectedRecipe.name}</span>
                    ...
                    {selectedRecipe && (
                        <div className={preselectedRecipe ? '' : 'animate-fade-in'}>
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                                {preselectedRecipe ? 'Seleziona giorni e pasti' : '2. Seleziona giorni e pasti'}
                            </p>
